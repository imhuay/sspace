...
![last modify](https://img.shields.io/static/v1?label=last%20modify&message=2025-12-27%2023%3A07%3A06&labelColor=gray&color=thistle&style=flat-square)
<!--END_SECTION:badge-->
<!--info
...
- Encoder 层 = **多头自注意力层** → **前馈网络层** (每层配 **残差连接** 与 **层归一化**)
- Decoder 层 = **掩码自注意力层** → **交叉注意力层** (Q 来自解码器, K/V 来自编码器) → **前馈网络层** (每层配 **残差连接** 与 **层归一化**)
...
> • **核心思想**: 通过 **自注意机制** 和 **位置编码**, 实现 **全局依赖建模** 和 **并行计算**; <br>
> • 解决了 RNN 的 **串行计算** 与 **长程依赖** 问题, 和 CNN 的 **局部感受野** 限制; <br>

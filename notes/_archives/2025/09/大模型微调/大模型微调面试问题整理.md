...
- **LoRA (Low-Rank Adaptation) / QLoRA (Quantized LoRA)**:
    > [LoRA 基础](./LoRA.md#基础概念)
    - **思路**:
        - 对需要微调的 **线性层** (如 `nn.Linear`), 冻结其原始权重 $W \in \mathbb{R}^{d_{\text{out}} \times d_{\text{in}}}$, 引入一个 **低秩增量** $\Delta W$, 用两个小矩阵参数化
            $$
        ...
        - **前向过程** (**旁路相加**):
            $$
            y = W x + \frac{\alpha}{r} \cdot B(Ax)
            $$
        - 缩放因子 $\frac{\alpha}{r}$ 控制 $\Delta W$ 的幅度, 避免训练初期过大扰动 (**软约束**);
    - **代码 Demo**:
        - [LoRa](./code/lora.py)
- **Adapter**:
    - **思路**:
